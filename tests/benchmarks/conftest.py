"""pytest-benchmark configuration for FastSIMUS benchmarks.

Adds an optional ``fast_simus_device_label`` entry to ``machine_info`` when the
``FASTSIMUS_DEVICE_LABEL`` environment variable is set. Lets cross-machine
benchmark runs carry a human-readable tag (e.g. "M2 Max MLX", "H100 JAX-CUDA")
that the scaling-plot script uses to distinguish series.
"""
