/*
 * IDEALIZED v21 CONSTFREQ -- exp19 idealized ceiling variant.
 *
 * Derived from simus_fused_v21_noatom.cu. The four per-subelement frequency
 * state arrays (GEO_KW_R, GEO_KR_STEP, GEO_ALPHA_R, GEO_AR_STEP) are
 * replaced by per-scatterer scalar registers captured once per scatterer
 * using the elem[0] / sub_dx[0] geometry. Shmem holds only GEO_AMP,
 * GEO_STP_RX_RE, GEO_STP_RX_IM (per-se, 3 arrays instead of 7).
 *
 * Purpose: isolate the effect of per-frequency state on shmem pressure /
 * occupancy. Numerically incorrect by design (ph/av no longer vary across
 * subelements); timing diagnostic only.
 *
 * Shmem layout:
 *   geo:   3 * B_SCAT * N_ES   floats  (AMP, STP_RX_RE, STP_RX_IM)
...
#define GEO_STP_RX_RE(s) (shmem + ((1*B_SCAT + (s)) * N_ES))
#define GEO_STP_RX_IM(s) (shmem + ((2*B_SCAT + (s)) * N_ES))
...
#define SH_TX_HALF_OFFSET (3 * B_SCAT * N_ES)
...
void simus_fused_kernel(
    ...
    /* CONSTFREQ: per-scatterer scalar registers replacing GEO_KW_R,
     * GEO_KR_STEP, GEO_ALPHA_R, GEO_AR_STEP. Captured from elem[0]/sub[0]. */
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        for (int si = 0; si < actual_b; si++) {
            ...
            /* CONSTFREQ: compute per-scatterer rc_0 from elem[0]/sub[0] geometry. */
            {
