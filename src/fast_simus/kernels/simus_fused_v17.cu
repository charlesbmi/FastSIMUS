/*
 * Fused TX+RX SIMUS kernel -- v17: warp-cooperative SCAT_REDUCE.
 *
 * Based on v11. In Phase 3, SCAT_REDUCE adjacent threads handle the SAME
 * frequency from DIFFERENT scatterer subsets. After per-frequency accumulation,
 * shuffle-reduce across SCAT_REDUCE lanes and only lane 0 atomicAdd.
 *
 * Thread mapping in Phase 3:
 *   scat_lane = threadIdx.x % SCAT_REDUCE
 *   freq_lane = threadIdx.x / SCAT_REDUCE
 *   FREQ_STRIDE = TG_SIZE / SCAT_REDUCE
 *   Thread handles scatterers: scat_lane, scat_lane+SR, scat_lane+2*SR, ...
 *
 * Each thread has ceil(B_SCAT/SCAT_REDUCE)*ELEM_TILE geometric progression
 * states (fewer registers than v11's B_SCAT*ELEM_TILE).
 *
 * Compile-time: N_ELEM, N_SUB, N_FREQ, N_ES, TILE_SE, TG_SIZE, MAX_FPT,
 *               B_SCAT, ELEM_TILE, SCAT_REDUCE, MY_B_SCAT
 *
 * MY_B_SCAT = ceil(B_SCAT / SCAT_REDUCE)
 *
...
#define FREQ_STRIDE (TG_SIZE / SCAT_REDUCE)
...
void simus_fused_kernel(
    ...
    /* Phase 1+2 use standard thread mapping */
    int my_n_freq = 0;
    ...
    /* Phase 3 SCAT_REDUCE mapping */
    const int scat_lane = lid % SCAT_REDUCE;
    const int freq_lane = lid / SCAT_REDUCE;
    const float freq_lane_f = (float)freq_lane;
    const float freq_stride_f = (float)FREQ_STRIDE;
    int my_n_freq_sr = 0;
    for (int f = freq_lane; f < N_FREQ; f += FREQ_STRIDE) my_n_freq_sr++;
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        /* ---- Phase 1+2: geometry + TX (unchanged from v11) ---- */
        for (int si = 0; si < actual_b; si++) {
            ...
            /* Phase 2: TX sweep (unchanged from v11) */
        ...
        /* ---- Phase 3: RX with SCAT_REDUCE warp-cooperative reduction ----
         *
         * Each SCAT_REDUCE group processes the same element group and frequency.
         * Each thread handles ceil(B_SCAT/SR) scatterers with geometric progression.
         * After accumulating per-frequency, shuffle-reduce and atomicAdd.
         */
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            /* Init geometric progression for assigned scatterers.
             * MY_B_SCAT = ceil(B_SCAT/SCAT_REDUCE) must be a compile-time define. */
            f2 rx_cv[MY_B_SCAT * ELEM_TILE];
            f2 rx_sv[MY_B_SCAT * ELEM_TILE];
            int my_scat_idx[MY_B_SCAT];
            int n_my_scat = 0;

            for (int si = scat_lane; si < actual_b; si += SCAT_REDUCE) {
                if (out_flag[si]) {
                    my_scat_idx[n_my_scat] = si;
                    #pragma unroll
                    for (int et = 0; et < ELEM_TILE; et++) {
                        int idx = n_my_scat * ELEM_TILE + et;
                        rx_cv[idx] = {0.0f, 0.0f};
                        rx_sv[idx] = {1.0f, 0.0f};
                    }
                    n_my_scat++;
                ...
                for (int et = 0; et < ELEM_TILE; et++) {
                    ...
                    int se = se_base + et;
                    float ph = GEO_KW_R(si)[se] + freq_lane_f * GEO_KR_STEP(si)[se];
                    float av = GEO_ALPHA_R(si)[se] + freq_lane_f * GEO_AR_STEP(si)[se];
                    float ai = GEO_AMP(si)[se] * expf(-av);
                    float vr, vi;
                    __sincosf(ph, &vi, &vr);
                    rx_cv[idx] = {vr * ai, vi * ai};

                    float stp_ph = freq_stride_f * GEO_KR_STEP(si)[se];
                    float stp_al = freq_stride_f * GEO_AR_STEP(si)[se];
                    float sm = expf(-stp_al);
                    float spr, spi;
                    __sincosf(stp_ph, &spi, &spr);
                    rx_sv[idx] = {spr * sm, spi * sm};
                }
                n_my_scat++;
            }

            /* Sweep frequencies with geometric progression */
            for (int fi = 0; fi < my_n_freq_sr; fi++) {
                int f = freq_lane + fi * FREQ_STRIDE;
                if (f >= N_FREQ) break;
                float pf = probe[f];
                ...
                for (int ms = 0; ms < n_my_scat; ms++) {
                    int si = my_scat_idx[ms];
                    float tk_re = sh_tx_re[si * N_FREQ + f];
                    float tk_im = sh_tx_im[si * N_FREQ + f];

                    #pragma unroll
                    for (int et = 0; et < ELEM_TILE; et++) {
                        int idx = ms * ELEM_TILE + et;
                        float rr = rx_cv[idx].x * inv_nsub;
                        float ri = rx_cv[idx].y * inv_nsub;
                        acc_re[et] += (tk_re*rr - tk_im*ri) * pf;
                        acc_im[et] += (tk_re*ri + tk_im*rr) * pf;
                        rx_cv[idx] = cmul(rx_cv[idx], rx_sv[idx]);
                    }
                }

                /* Warp shuffle reduction across SCAT_REDUCE lanes */
                #pragma unroll
                for (int et = 0; et < ELEM_TILE; et++) {
                    #pragma unroll
                    for (int offset = SCAT_REDUCE / 2; offset > 0; offset >>= 1) {
                        acc_re[et] += __shfl_xor_sync(0xffffffff, acc_re[et], offset);
                        acc_im[et] += __shfl_xor_sync(0xffffffff, acc_im[et], offset);
                    }
                }
