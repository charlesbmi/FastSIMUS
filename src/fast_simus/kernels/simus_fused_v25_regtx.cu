/*
 * Fused TX+RX SIMUS kernel -- v25: register-resident TX (correct, fp32).
 *
 * Based on v11. KEY CHANGE: TX buffer is moved out of shared memory and
        ...
        for (int si = 0; si < actual_b; si++) {
...

            {
                ...
                for (int f = lid; f < N_FREQ; f += TG_SIZE, fi++) {
                    ...
                    tk_re[si * MAX_FPT + fi] = (ppr*tr - ppi*ti) * rc;
                    tk_im[si * MAX_FPT + fi] = (ppr*ti + ppi*tr) * rc;
                }
                /* Pad remaining slots so out-of-range freqs have zero TX. */
                #pragma unroll
                for (; fi < MAX_FPT; fi++) {
                    tk_re[si * MAX_FPT + fi] = 0.0f;
                    tk_im[si * MAX_FPT + fi] = 0.0f;
