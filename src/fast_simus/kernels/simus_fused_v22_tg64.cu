/*
 * v22_tg64 -- exp20 6-blocks/SM variant.
 *
 * Base: simus_fused_v21_idealized.cu (CONSTFREQ + SINGLEELEM + no-atom).
 * Lever: TG_SIZE=64 + __launch_bounds__(64, 6). At TG=64 the per-block
 * register budget is 65536 / (6 * 64) = 170 regs/thread. Halves the
 * warp pool (24 warps/SM instead of 48) which may regress the wait stall.
 *
