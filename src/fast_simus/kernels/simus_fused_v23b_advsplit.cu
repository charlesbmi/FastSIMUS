/*
 * Fused TX+RX SIMUS kernel -- v23b: advance-split Phase 3 (correct, fp32).
 *
 * Based on v11. Lighter-weight restructure than v23: keeps cv reads
 * inline in the FMA stage (no rr/ri scratch arrays), but splits the
 * chain advance `cv = cmul(cv, sv)` into its own pass at the end of
 * each freq iteration. Goal: break the per-iteration FMA-then-cmul RAW
 * hazard without doubling the scratch register footprint that v23 imposed.
 *
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                /* Stage A: FMA accumulation, cv read inline (no scratch). */
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
                ...
                /* Stage B: chain advance, separate pass. */
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
                    #pragma unroll
                    for (int et = 0; et < ELEM_TILE; et++) {
