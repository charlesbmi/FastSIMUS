/*
 * Fused TX+RX SIMUS kernel -- v12: per-block private output buffers.
 *
 * ZERO atomics. Each block accumulates into its own region of global memory:
 *   output_block[blockIdx.x][elem * N_FREQ + freq]
 * A separate reduction kernel sums the per-block outputs.
 *
 * Based on v11 (batched accum + shmem TX + element tiling), but replaces
 * atomicAdd with regular read-add-write to private output.
 *
...
void simus_fused_kernel(
    ...
    /* Per-block output: offset into the per-block region of spect_re/spect_im.
     * Layout: spect_re[blockIdx.x * N_ELEM * N_FREQ + elem * N_FREQ + freq] */
    const int out_stride = N_ELEM * N_FREQ;
    float* my_ore = spect_re + blockIdx.x * out_stride;
    float* my_oim = spect_im + blockIdx.x * out_stride;
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        /* KEY CHANGE: write to per-block private output (no atomics) */
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                for (int et = 0; et < ELEM_TILE; et++) {
                    ...
                    int out_idx = elem * N_FREQ + f;
                    my_ore[out_idx] += acc_re[et];
                    my_oim[out_idx] += acc_im[et];
                }
            }
        }
...
/* Reduction kernel: sum per-block outputs into final output.
 * Grid: ceil(N_ELEM * N_FREQ / 256), Block: 256
 * Reads from per_block_re/im[n_blocks][N_ELEM * N_FREQ],
 * writes to final_re/im[N_ELEM * N_FREQ]. */
extern "C" __global__
void reduce_blocks_kernel(
    const float* __restrict__ per_block_re,
    const float* __restrict__ per_block_im,
    float* __restrict__ final_re,
    float* __restrict__ final_im,
    int n_blocks,
    int output_size
) {
    ...
    for (int b = 0; b < n_blocks; b++) {
        sum_re += per_block_re[b * output_size + idx];
        sum_im += per_block_im[b * output_size + idx];
    }
    final_re[idx] = sum_re;
    final_im[idx] = sum_im;
}
