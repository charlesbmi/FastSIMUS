/*
 * Fused TX+RX SIMUS kernel -- v13: frequency-chunked.
 *
 * Same architecture as v11 but processes only N_FREQ frequencies per launch.
 * Multiple launches cover the full frequency range.
 * freq_offset shifts the initial phase/attenuation to the chunk's starting freq.
 * n_freq_total is the full frequency count (for output array indexing).
 *
 * Benefits:
 * - Smaller TX buffer (2 * B_SCAT * N_FREQ) where N_FREQ = chunk_size
 * - Lower shmem -> higher B_SCAT and/or more blocks/SM
 * - Fewer atomics per launch (fewer output locations per scatterer)
 *
 * Compile-time: N_ELEM, N_SUB, N_FREQ (=chunk_size), N_ES, TILE_SE, TG_SIZE,
 *               MAX_FPT, B_SCAT, ELEM_TILE
 */
...
void simus_fused_kernel(
    ...
    int lid = threadIdx.x;
    float base_f = (float)(freq_offset + lid);
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        for (int si = 0; si < actual_b; si++) {
            ...
            /* Phase 2: TX sweep -- starts at base_f, steps by stride_f */
            ...
            for (int tile = 0; tile < N_TILES; tile++) {
                ...
                for (int j = 0; j < TILE_SE; j++) {
                    ...
                    float ph = GEO_KW_R(si)[se] + base_f * GEO_KR_STEP(si)[se];
                    float av = GEO_ALPHA_R(si)[se] + base_f * GEO_AR_STEP(si)[se];
                    float ai = GEO_AMP(si)[se] * expf(-av);
                    float vr, vi;
                    __sincosf(ph, &vi, &vr);
                    vr *= ai; vi *= ai;
                    float dp = base_f * sh_dps_l[em];
                    float dr, di;
                    __sincosf(dp, &di, &dr);
        ...
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                for (int et = 0; et < ELEM_TILE; et++) {
                    ...
                    atomicAdd(&spect_re[elem * n_freq_total + gf], acc_re[et]);
                    atomicAdd(&spect_im[elem * n_freq_total + gf], acc_im[et]);
                }
            }
        }
