/*
 * v22_nochain_ilp -- exp20 ILP-restructured Phase 3.
 *
 * Base: v22_nochain.
 * Lever: Phase 3 inner body split into two explicit stages:
 *   Stage 1 (prep):  for all si in 0..B, issue __sincosf/expf/TX-load
 *                    back-to-back into per-si scratch regs. No RAW
 *                    dependencies between different si, so the scheduler
 *                    has B independent SFU dispatches in flight at once.
 *   Stage 2 (fma):   consume the prepared (rr, ri, tk_re, tk_im) with
 *                    the ET accumulation FMAs.
 *
 * Makes the compiler's implicit reordering explicit, so we can measure
 * whether warps_active rises above the v22_nochain 16.5% floor.
 * Extra register pressure: 4*B_SCAT floats per thread.
 *
...
void simus_fused_kernel(
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        /* Phase 3 (nochain_ilp): two-stage, batched-SFU structure. */
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                /* Stage 1: batch all SFU + TX-load per-si into scratch. */
                float rr_arr[B_SCAT], ri_arr[B_SCAT];
                float tkr_arr[B_SCAT], tki_arr[B_SCAT];
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
                    if (si >= actual_b || out_flag[si]) {
                        rr_arr[si]  = 0.0f; ri_arr[si]  = 0.0f;
                        tkr_arr[si] = 0.0f; tki_arr[si] = 0.0f;
                        continue;
                    }
                    tkr_arr[si] = h2f(sh_tx_re[si * N_FREQ + f]);
                    tki_arr[si] = h2f(sh_tx_im[si * N_FREQ + f]);
                    ...
                    rr_arr[si] = vr * ai * inv_nsub;
                    ri_arr[si] = vi * ai * inv_nsub;
                }

                /* Stage 2: consume scratch via ET-unrolled FMAs. */
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
