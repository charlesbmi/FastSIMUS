/*
 * v22_nochain -- exp20 6-blocks/SM variant.
 *
 * Base: simus_fused_v21_idealized.cu (CONSTFREQ + SINGLEELEM + no-atom).
 * Lever: Phase 3 breaks the cv/sv cmul running state. Instead of carrying
 * cv[B*ET] and sv_arr[B*ET] live across the per-freq loop, cv is resynth
 * via __sincosf(ph_f, ...) per frequency. Live register footprint drops
 * from ~2*2*B*ET plus temporaries down to B*ET temporaries per iter.
 * Compute volume goes up (extra sincosf per freq), preserving DCE sanity.
 *
...
void simus_fused_kernel(
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        /* Phase 3 (nochain): resynth cv per freq, no live chain state. */
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                for (int si = 0; si < B_SCAT; si++) {
                    ...
                    /* Freshly synthesize rotation at freq f -- NO live chain. */
