/*
 * v11-stagger: same as v11 but element groups are processed in staggered
 * order across blocks to reduce L2 atomic cache line contention.
 *
 * Block b starts at element group (b % N_ELEM_GROUPS) instead of 0.
 * This distributes atomic writes across different cache lines at any given
 * point in time, reducing serialization on the L2 atomic pipelines.
 */
...
void simus_fused_kernel(
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        /* Phase 3: STAGGERED element group order to reduce atomic contention.
         * Block b starts at element group (b % N_ELEM_GROUPS). */
        for (int ego = 0; ego < N_ELEM_GROUPS; ego++) {
            int eg = (ego + blockIdx.x) % N_ELEM_GROUPS;
