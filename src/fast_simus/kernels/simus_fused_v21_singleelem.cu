/*
 * IDEALIZED v21 SINGLEELEM -- exp19 idealized ceiling variant.
 *
 * Derived from simus_fused_v21_noatom.cu. All per-subelement (se) geometry
 * variation is eliminated by pretending every element sits at elem[0] with
 * sub_dx[0]/sub_dz[0]. The loop over se is preserved (compute volume
 * maintained) but the shmem GEO_* arrays collapse to a single float per
 * scatterer. Delay arrays collapse to 3 scalars.
 *
 * Purpose: isolate the effect of per-element shmem array size on occupancy
 * and bank pressure. Numerically incorrect by design; timing diagnostic only.
 *
 * Shmem layout:
 *   geo:   7 * B_SCAT             floats  (per-scatterer scalars)
 *   tx_re: B_SCAT * N_FREQ         ushort  (fp16 bits)
 *   tx_im: B_SCAT * N_FREQ         ushort  (fp16 bits)
 *   delay: 3                       floats  (single pair for elem[0])
 */
...
/* Per-scatterer scalar shmem (one float per scatterer, no per-se index). */
#define GEO_AMP(s)       (shmem + (0*B_SCAT + (s)))
#define GEO_KW_R(s)      (shmem + (1*B_SCAT + (s)))
#define GEO_KR_STEP(s)   (shmem + (2*B_SCAT + (s)))
#define GEO_ALPHA_R(s)   (shmem + (3*B_SCAT + (s)))
#define GEO_AR_STEP(s)   (shmem + (4*B_SCAT + (s)))
#define GEO_STP_RX_RE(s) (shmem + (5*B_SCAT + (s)))
#define GEO_STP_RX_IM(s) (shmem + (6*B_SCAT + (s)))
...
#define SH_TX_HALF_OFFSET (7 * B_SCAT)
...
void simus_fused_kernel(
    ...
    /* SINGLEELEM: delay arrays collapse to 3 scalars (one per array). */
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        for (int si = 0; si < actual_b; si++) {
            ...
            /* SINGLEELEM: loop over all se preserved (compute volume), but
             * every iteration uses elem[0]/sub[0] geometry so the stored
             * values are identical. All threads race-write the same scalar
             * per scatterer -- benign since the value is the same. */
            for (int se = lid; se < N_ES; se += TG_SIZE) {
                ...
                GEO_KW_R(si)[0]    = kw_init * rc_;
                GEO_KR_STEP(si)[0] = kw_step * rc_;
                GEO_ALPHA_R(si)[0] = alpha_init * rc_;
                GEO_AR_STEP(si)[0] = alpha_step * rc_;
            ...
            for (int tile = 0; tile < N_TILES; tile++) {
                ...
                for (int j = 0; j < TILE_SE; j++) {
                    ...
                    float ph = GEO_KW_R(si)[0] + lid_f * GEO_KR_STEP(si)[0];
                    float av = GEO_ALPHA_R(si)[0] + lid_f * GEO_AR_STEP(si)[0];
