/*
 * Fused TX+RX SIMUS kernel -- v15: fp16 TX buffer in shared memory.
 *
 * Based on v11. The TX buffer (dominant shmem cost at ~78%) is stored
 * as unsigned short (fp16 bits) instead of float, halving its footprint.
 * This allows higher B_SCAT while maintaining >= 2 blocks/SM.
 *
 * Shmem layout:
