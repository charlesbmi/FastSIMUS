/*
 * Fused TX+RX SIMUS kernel -- v23: chain-split Phase 3 (correct, fp32 TX).
 *
 * Based on v11 (fp32 TX). Single change vs v11: Phase 3 inner body
 * is split into three explicit stages per freq:
 *   Stage 1 (prep):    snapshot cv -> rr/ri scratch + load TX into tk regs
 *   Stage 2 (FMA):     accumulate acc_re/im, no chain advance interleaved
 *   Stage 3 (advance): cv = cmul(cv, sv_arr) for all (si, et)
 *
 * Motivation (exp20): the implicit interleaving of FMA + cmul-chain
 * advance in v11's `for fi -> for si -> for et` body produced
 * ~13% of redundant ALU instructions in nvcc's emit, on top of the
 * RAW hazard that forced FMAs to wait on the previous (si, et)'s cmul.
 * Splitting the body removes both costs while preserving the chain
 * (and therefore numerical correctness vs v11).
 *
...
void simus_fused_kernel(
         ...
         scat_base += gridDim.x * B_SCAT)
    {
        ...
        for (int eg = 0; eg < N_ELEM_GROUPS; eg++) {
            ...
            /* Initialize B_SCAT * ELEM_TILE RX states */
             ...
             * Three-stage body: prep -> FMA -> advance, then sink.        */
            for (int fi = 0; fi < my_n_freq; fi++) {
                ...
                /* Stage 1 (prep): snapshot current cv state into rr/ri,
                 * load TX into per-si tk regs. No writes to cv yet. */
                float rr_arr[B_SCAT * ELEM_TILE];
                float ri_arr[B_SCAT * ELEM_TILE];
                float tkr_arr[B_SCAT];
                float tki_arr[B_SCAT];
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
                    tkr_arr[si] = sh_tx_re[si * N_FREQ + f];
                    tki_arr[si] = sh_tx_im[si * N_FREQ + f];
                    #pragma unroll
                    for (int et = 0; et < ELEM_TILE; et++) {
                        int idx = si * ELEM_TILE + et;
                        rr_arr[idx] = cv[idx].x * inv_nsub;
                        ri_arr[idx] = cv[idx].y * inv_nsub;
                    }
                }
                ...
                #pragma unroll
                for (int si = 0; si < B_SCAT; si++) {
                    float tk_re = tkr_arr[si];
                    float tk_im = tki_arr[si];
                    #pragma unroll
                    for (int et = 0; et < ELEM_TILE; et++) {
                        int idx = si * ELEM_TILE + et;
                        float rr = rr_arr[idx];
