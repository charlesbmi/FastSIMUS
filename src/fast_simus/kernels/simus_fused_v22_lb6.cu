/*
 * v22_lb6 -- exp20 6-blocks/SM variant.
 *
 * Base: simus_fused_v21_idealized.cu (CONSTFREQ + SINGLEELEM + no-atom).
 * Lever: __launch_bounds__(128, 6) forces the compiler to size the
 * register allocation so 6 blocks of 128 threads co-reside on the SM
 * (65536 / (6 * 128) = 85 regs/thread max). Expect heavy spill.
 *
